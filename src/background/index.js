const GITHUB_API_URL =
  "https://raw.githubusercontent.com/axthauvin/better-zeus/main/version.json";
  ...
  try {
    const result = await new Promise((resolve) =>
      chrome.storage.local.get(
        ["lastUpdateCheck", "latestVersionCache"],
        resolve,
      ),
    );
    ...
    if (
      ...
      now - result.lastUpdateCheck > 3600000 ||
      !latestVersion
    ) {
      ...
      latestVersion = latestTag.startsWith("v")
    ...
    console.error(
      "Failed to check for Better Zeus updates:",
      error.message || error,
    );
  }
}
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (
    ...
    tab.url &&
    tab.url.includes("zeus.ionis-it.com")
  ) {
